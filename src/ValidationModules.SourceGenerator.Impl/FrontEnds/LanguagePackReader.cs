...
public static class LanguagePackReader
{
    ...
    public static readonly ImmutableDictionary<string, int> ShapeInventory = new Dictionary<
        string,
        int
    ...
    public static Outcome Read(LanguagePackFile file, int index)
    {
        ...
        if (
            FileNameCulture(file.Path) is { } named
            && !string.Equals(named, culture, StringComparison.OrdinalIgnoreCase)
        )
        {
            diagnostics.Add(
                Diagnostic.Create(
                    ValidationDiagnostics.LanguagePackNameMismatch,
                    at,
                    PackName(file.Path),
                    named,
                    culture
        ...
        foreach (var (key, template) in entries)
        {
            if (!seen.Add(key))
            {
                diagnostics.Add(
                    Diagnostic.Create(
                        ValidationDiagnostics.LanguagePackDuplicateKey,
                        at,
                        key,
                        PackName(file.Path)
                    )
            ...
            if (ShapeInventory.TryGetValue(key, out var arity))
            {
                if (HighestHole(template) is { } hole && hole >= arity)
                {
                    diagnostics.Add(
                        Diagnostic.Create(
                            ValidationDiagnostics.LanguagePackHoleOutOfRange,
                            at,
                            key,
                            hole,
                            arity,
                ...
                key.IndexOf('.') is var dot
                && dot > 0
                && KnownCodes.Contains(key.Substring(0, dot))
            )
            {
                diagnostics.Add(
                    Diagnostic.Create(
                        ValidationDiagnostics.LanguagePackUnknownShape,
                        at,
                        key,
                        PackName(file.Path),
                        NearestKey(key)
        ...
        var missing = ShapeInventory
            .Keys.Where(key => !seen.Contains(key))
        ...
        if (missing.Count > 0 && kept.Count > 0)
        {
            diagnostics.Add(
                Diagnostic.Create(
                    ValidationDiagnostics.LanguagePackCoverage,
                    at,
                    culture,
                    ShapeInventory.Count - missing.Count,
                    ShapeInventory.Count,
    ...
    private static Location FileLocation(string path) => Location.Create(path, default, default);
    ...
    private static string? FileNameCulture(string path)
    {
        ...
        return
            token.Length is >= 2 and <= 11
            && token.All(c => char.IsLetter(c) || c == '-')
