...
public static class RuntimeContract
{
    ...
    // 11 -> 12: [StringLength] reads as DataAnnotations reads it. The one positional argument is
    // the maximum, the minimum is only ever named, and the two-argument constructor is gone. A
    ...
    //
    // Not additive, like 4 -> 5: a constructor is removed, which the number cannot express. Nor
    // does it cover the other direction. An Impl compiled at contract 11 reads the one-argument
    // form as no bound at all, and a contract-12 runtime satisfies its check, so that pairing is
    // written down here rather than caught.
