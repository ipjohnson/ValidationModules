...
/// <c>TReporter = ValidationContext</c>), so emitted code did not change shape. The receiver stays
/// by value because an <c>in</c> or <c>ref</c> receiver would refuse
/// <c>context.Push("home").ReportRequired(...)</c> - the result of a call is not addressable.
/// </para>
/// <para>
...
/// the literal, because at that point the text is one the author chose rather than one this file
/// owns.
/// </para>
/// <para>
...
public static class ValidationContextExtensions {
    ...
    /// as the absent bound. The extreme is not a bound anyone wrote, and composing it into the
    /// message quotes it back to the caller - a specification setting only <c>minimum</c> produced
