...
public static class EngineParity
{
    ...
    public static string? DataAnnotationsDivergence()
    {
        ...
        if (failing != expected)
        {
            return $"DataAnnotations found {failing} failures where the other engines find {expected}. "
    ...
    private static int VmCount<T>(IValidatorFor<T> validator, T value) =>
        validator.Validate(value).Errors.Count;

    private static int FvCount<T>(IValidator<T> validator, T value) =>
        validator.Validate(value).Errors.Count;
