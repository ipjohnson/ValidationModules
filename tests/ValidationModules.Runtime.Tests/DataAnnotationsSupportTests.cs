...
public class DataAnnotationsSupportTests
{
    private sealed class EvenAttribute : ValidationAttribute
    {
        public override bool IsValid(object? value) =>
            value is null || (value is int n && n % 2 == 0);
    }
    ...
    {
        protected override DataAnnotationsResult? IsValid(
            object? value,
            DataAnnotationsContext validationContext
        ) =>
    ...
    private sealed class Provider : IServiceProvider
    {
        public object? GetService(Type serviceType) =>
            serviceType == typeof(string) ? "here" : null;
    }
    ...
    public void Validate_MatchesWhatTryValidateObjectSays()
    {
        ...
        var daContext = new DataAnnotationsContext(model)
        {
            MemberName = "Count",
            DisplayName = "Count",
        };
    ...
    public void IsValid_AgreesWithValidateOnBothKindsOfAttribute()
    {
        ...
        Assert.True(
            DataAnnotationsSupport.IsValid(new EvenAttribute(), payload, 4, "Count", "Count")
        );
        Assert.False(
            DataAnnotationsSupport.IsValid(new EvenAttribute(), payload, 3, "Count", "Count")
        );
        ...
        Assert.False(
            DataAnnotationsSupport.IsValid(
                new NeedsServiceAttribute(),
                payload,
                1,
                "Count",
    ...
    public void Apply_NullMessage_ComposesAFallbackRatherThanAnEmptyError()
    {
        ...
        DataAnnotationsSupport.Apply(
            ref context,
            new DataAnnotationsResult(null),
            "count",
            CamelCaseFieldNamer.Instance
        );
    ...
    public void ValidateObject_MapsEveryYieldedResult()
    {
        ...
        var flow = DataAnnotationsSupport.ValidateObject(
            ref context,
            new SelfChecking { Start = 5, End = 1 },
            CamelCaseFieldNamer.Instance
        );
    ...
    public void ValidateObject_CleanObject_ReportsNothing()
    {
        ...
        DataAnnotationsSupport.ValidateObject(
            ref context,
            new SelfChecking { Start = 1, End = 5 },
            CamelCaseFieldNamer.Instance
        );
