...
public class RuleTextTests
{
    ...
    public void AnchorOfPredicate_TakesTheFirstMemberOfTheParameter(
        string predicate,
        string expected
    )
    {
    ...
    public void RenderPredicate_StripsTheParameterAndNamesTheMembers(
        string predicate,
        string expected
    )
    {
    ...
    public void CodeOfPredicate_FollowsTheWireNameRatherThanTheClrName()
    {
        ...
        static string Pinned(string clr) =>
            clr == "BeginsAt" || clr == "Start" ? "start" : Name(clr);
    ...
    public void CodeOfPredicate_ReadsALiteralAsPartOfTheRule()
    {
        ...
        Assert.Equal(
            "status_equal_active",
            RuleText.CodeOfPredicate("x => x.Status == \"active\"", Name)
        );
        Assert.Equal(
            "status_equal_pending",
            RuleText.CodeOfPredicate("x => x.Status == \"pending\"", Name)
        );
    }
    ...
    public void CodeOfPredicate_SplitsHumpsAndAcronymsAndDropsStructure(
        string predicate,
        string expected
    )
    {
    ...
    public void CodeOfPredicate_AnIdiomInsideALargerRule_KeepsTheRest()
    {
        ...
        Assert.Equal(
            "paid_and_name_is_not_null_or_empty",
            RuleText.CodeOfPredicate("x => x.Paid && !string.IsNullOrEmpty(x.Name)", Name)
        );

        Assert.Equal(
            "items_count_greater_than_0_and_paid",
