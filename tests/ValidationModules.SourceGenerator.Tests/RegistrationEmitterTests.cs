...
public class RegistrationEmitterTests
{
    ...
    public void DependencyModulesBranch_EmitsAModuleWrappingTheSameExtension()
    {
        ...
        Snapshot.Match(
            Registration(compiles: false, ("ValidationModules_Registration", "DependencyModules"))
        );
    }
    ...
    public void NoneEmitsNothingAtAll()
    {
        ...
        Assert.Equal(
            "<no registration emitted>",
            Registration(("ValidationModules_Registration", "None"))
        );
    }
    ...
    public void TheRegisteredNamerMatchesThePolicyTheLiteralsWereEmittedWith(
        string? policy,
        string expected
    )
    {
    ...
    )
    {
        ...
        var source = GeneratorHarness.Run(TwoTypes, assemblyName).Sources[
