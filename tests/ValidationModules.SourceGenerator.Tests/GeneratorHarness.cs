...
public static class GeneratorHarness
{
    ...
    )
    {
        ...
        var texts = (additionalFiles ?? Array.Empty<(string, string)>())
            .Select(static file =>
                (AdditionalText)new InMemoryAdditionalText(file.Item1, file.Item2)
            )
            .ToImmutableArray();
        ...
        var sources = output
            .SyntaxTrees.Where(tree => tree.FilePath.EndsWith(".g.cs", StringComparison.Ordinal))
            .ToDictionary(tree => Path.GetFileName(tree.FilePath), tree => tree.ToString());
