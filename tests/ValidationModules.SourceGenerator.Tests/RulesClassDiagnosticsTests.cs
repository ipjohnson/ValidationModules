...
public class RulesClassDiagnosticsTests
{
    ...
    public void ValueThatIsNotAMemberPath_IsVM3007(string statement)
    {
        ...
        Assert.Equal(
            ...
            Assert.Single(result.Diagnostics, d => d.Id == "VM3007").Severity
        );
    }
    ...
    public void EnsureReadingNoProperty_IsVM3102()
    {
        ...
        Assert.Equal(
            DiagnosticSeverity.Error,
            Assert.Single(result.Diagnostics, d => d.Id == "VM3102").Severity
        );
    }
    ...
    [InlineData(
        "Helper(rules);",
        "    private static bool Helper(ValidationRules<Reservation> r) => true;\n"
    )]
    [InlineData(
        "System.Func<ValidationModules.PropertyRules<Reservation, string?>> f = () => rules.Require(x.Guest);"
    )]
    ...
    [Theory]
    [InlineData(
        "    private static bool Allowed(string? s) => true;\n",
