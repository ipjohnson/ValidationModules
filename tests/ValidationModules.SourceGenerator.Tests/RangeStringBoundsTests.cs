...
public class RangeStringBoundsTests
{
    ...
    [InlineData(
        "[Range(\"2000-01-01T00:00:00+00:00\", \"2100-01-01T00:00:00+00:00\")] public DateTimeOffset At { get; init; }"
    )]
    [InlineData(
        "[Range(\"2000-01-01\", \"2100-01-01\")] public DateOnly? MaybeBorn { get; init; }"
    )]
    ...
    public void DateOnlyBounds_BecomeConstructorCalls()
    {
        var result = GeneratorHarness.Run(
            Model("[Range(\"2000-01-15\", \"2100-12-31\")] public DateOnly Born { get; init; }")
        );
    ...
    {
        var result = GeneratorHarness.Run(
            Model("[Range(\"0.00\", \"9.99\")] public decimal Price { get; init; }")
        );
    ...
    public void TimeSpanBounds_KeepDaysAndMilliseconds()
    {
        ...
        Assert.Contains(
            "new global::System.TimeSpan(1, 2, 3, 4, 5)",
    ...
    public void StringBounds_AppearInTheMessageAsWellAsTheComparison()
    {
        ...
        var result = GeneratorHarness.Run(
            Model("[Range(\"2000-01-01\", \"2100-01-01\")] public DateOnly Born { get; init; }")
        );

        var emitted = result.Sources["Sample.PetValidator.g.cs"];
        var occurrences =
            emitted
                .Split(["new global::System.DateOnly(2000, 1, 1)"], StringSplitOptions.None)
                .Length - 1;
    ...
    {
        var result = GeneratorHarness.Run(
            Model("[Range(\"not-a-date\", \"2100-01-01\")] public DateOnly Born { get; init; }")
        );
    ...
    public void StringBoundsOnAStringMember_AreVM1003AndNotAlsoVM1103()
    {
        ...
        var result = GeneratorHarness.Run(
            Model("[Range(\"a\", \"z\")] public string? Grade { get; init; }")
        );
    ...
    public void IntBoundsOnADecimal_StillWorkWithoutBeingRewritten()
    {
        ...
        var result = GeneratorHarness.Run(
            Model("[Range(0, 30)] public decimal Price { get; init; }")
        );
    ...
    public void ExclusiveBoundsSurviveTheRewrite()
    {
        ...
        Assert.Contains(
            ">= new global::System.DateOnly(2100, 1, 1)",
            result.Sources["Sample.PetValidator.g.cs"]
        );
        Assert.Empty(result.CompilationErrors);
    }
