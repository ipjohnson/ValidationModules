...
| `As<TFacet>(TFacet value)` | Runs the rules declared for an interface or base type of `x`, at the current level, including the constraint attributes on its properties. The type's own checks leave those attributes out. The argument must be `x`. The type of `x` itself is reported as `VM3110`. |
