...
public class LiftedConstantTests
{
    ...
    public void ADoubleConstantKeepsEveryDigit()
    {
        ...
        Assert.Equal(
            "ratio",
            Assert
                .Single(Validator.Validate(Valid() with { Ratio = 1.2345678901234569 }).Errors)
                .Code
        );
    }
    ...
    public void AnEnumConstantStillNamesItsMember()
    {
        ...
        Assert.True(
            Validator.Validate(Valid() with { Amount = 0m, Tier = QuoteTier.Standard }).IsValid
        );
        ...
        Assert.Equal(
            ...
            Assert
                .Single(
                    Validator
                        .Validate(Valid() with { Amount = 0m, Tier = QuoteTier.Premium })
                        .Errors
                )
                .Code
        );
